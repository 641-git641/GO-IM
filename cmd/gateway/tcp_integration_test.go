...
// This file contains additional integration tests for gnet TCP transport.
// It is compiled as part of the main package.
...
// startTestServerGNet starts the server with both transports for TCP testing.
func startTestServerGNet(t *testing.T) (*App, func()) {
	t.Helper()
	cfg := configs.Default()
	cfg.Gateway.HTTPAddr = ":18080" // HTTP/WebSocket
	cfg.Gateway.TCPAddr = ":18081"  // gnet TCP
	cfg.Gateway.Transport = "both"  // enable both transports
	...
	// Wait for server to be ready (both HTTP and gnet).
	...
	return app, func() {
		cancel()
		// Wait for both HTTP and gnet ports to be released.
		for i := 0; i < 30; i++ {
			time.Sleep(100 * time.Millisecond)
			// Check HTTP port.
...
// connectTCP dials the gnet TCP port.
...
// readTCPFrame reads a single framed protobuf message from a TCP connection.
...
// writeTCPFrame sends a proto.Message as a framed TCP message.
...
// ---------- gnet TCP integration tests ----------

// TestIntegrationGNetEndToEnd tests a full gnet TCP flow: connect -> login -> chat -> ACK.
func TestIntegrationGNetEndToEnd(t *testing.T) {
	_, cleanup := startTestServerGNet(t)
	defer cleanup()

	// Get JWT tokens via HTTP (same server).
	...
	// Alice connects via gnet TCP.
	aliceConn := connectTCP(t, "localhost:18081")
	defer aliceConn.Close()

	// Bob connects via WebSocket (mixed transport test).
	bobConn := connectWS(t, bobToken)
	defer bobConn.Close()

	// Drain Bob's login response (WebSocket).
	...
	// Alice sends login frame via TCP.
	writeTCPFrame(t, aliceConn, &proto.Message{
		Cmd:     proto.CmdLogin,
		Content: aliceToken,
	})

	// Alice reads login response via TCP.
	...
	// Alice sends a chat message to Bob via TCP.
	writeTCPFrame(t, aliceConn, &proto.Message{
		Cmd:      proto.CmdChat,
		To:       bobUID,
	...
	// Bob receives the message via WebSocket.
...
// TestIntegrationGNetOfflineMessage tests offline message delivery via gnet TCP.
func TestIntegrationGNetOfflineMessage(t *testing.T) {
	...
	// Alice sends message to offline Bob via TCP.
	writeTCPFrame(t, aliceConn, &proto.Message{
		Cmd:      proto.CmdChat,
		To:       bobUID,
		ChatType: proto.ChatTypeSingle,
	...
	// Bob requests offline messages via TCP.
	writeTCPFrame(t, bobConn, &proto.Message{
		Cmd: proto.CmdOffline,
	})

	// Bob receives the offline message via TCP.
...
// TestIntegrationGNetHeartbeat tests TCP application-level heartbeat.
func TestIntegrationGNetHeartbeat(t *testing.T) {
	...
	// Send a heartbeat via TCP.
	writeTCPFrame(t, conn, &proto.Message{
		Cmd: proto.CmdHeartbeat,
	})

	// Receive heartbeat response via TCP.
