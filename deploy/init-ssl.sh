...
echo " ✅ SSL 证书初始化完成！"
echo ""
echo " 接下来请执行（详见 README 生产部署章节）："
echo "  1. 生成凭据写入 .env（MYSQL_ROOT_PASSWORD / MYSQL_PASSWORD / MINIO_ROOT_PASSWORD）"
echo "  2. 渲染生产配置:"
echo "     DOMAIN=$DOMAIN JWT_SECRET=<随机64字符> MYSQL_PASSWORD=<同.env> \\"
echo "     MINIO_SECRET_KEY=<同.env> ADMIN_UID=<你的UID> bash deploy/render-config.sh"
echo "  3. 启动: docker compose -f docker-compose.prod.yml up -d"
echo "  4. 验证: curl -I https://$DOMAIN/health"
echo "============================================"
